...
<head>
    ...
    <title>Wahyu Audio Project - Pro DSP v9.3 (RTA + Phase Checker)</title>
    <style>
        :root {
            --bg-base: #0a0a0c; 
            --bg-panel: #13151a; 
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; touch-action: manipulation; }
        ...
        /* Phase Checker Badge & Box */
        .phase-check-box { display: flex; align-items: center; justify-content: space-between; background: #08090a; padding: 10px 14px; border-radius: var(--radius-sm); border: 1px solid #222; margin-bottom: 18px; }
        .phase-status { font-weight: 900; font-size: 0.75rem; font-family: monospace; letter-spacing: 1px; display: flex; align-items: center; gap: 8px; color: var(--text-muted); }
        .phase-led { width: 10px; height: 10px; border-radius: 50%; background: #333; display: inline-block; }
        .phase-led.in-phase { background: var(--led-green); box-shadow: 0 0 8px var(--led-green); }
        .phase-led.out-phase { background: var(--led-red); box-shadow: 0 0 8px var(--led-red); }
        .phase-led.checking { background: var(--led-yellow); box-shadow: 0 0 8px var(--led-yellow); }
        ...
        .val-input-group { display: flex; align-items: center; gap: 4px; margin-top: 10px; background: #08090a; padding: 4px 10px; border-radius: 12px; border: 1px solid #222;}
        ...
        input[type="range"] { width: 100%; height: 4px; background: #222; outline: none; -webkit-appearance: none; border-radius: 2px; border: none; margin: 12px 0; }
        input[type="range"]::-webkit-slider-thumb { 
            ...
            box-shadow: 0 2px 6px rgba(0,0,0,0.8); transition: 0.1s; 
        }
        ...
        .peq-band-title { font-size: 0.75rem; color: var(--accent-brand); text-align: center; margin-bottom: 12px; font-weight: 900; letter-spacing: 1px; background: #08090a; padding: 6px; border-radius: 4px;}
...
<div class="container">
        ...
        <div class="grid-2col">
            <div class="module-box">
                ...
                <div style="margin-top:25px; padding-top:15px; border-top: 1px dashed var(--border);">
                    ...
                    <div style="display:flex; justify-content: space-between; align-items:center; background: #050505; padding:12px; border-radius:var(--radius-sm); border:1px solid #222;">
                        ...
                        <div style="display:flex; flex-direction:column; gap:6px;">
                            <button class="btn-link" onclick="applyCalculatedDelay('l')" style="padding:6px 10px; font-size:0.65rem;">SET CH A</button>
                            <button class="btn-link" onclick="applyCalculatedDelay('r')" style="padding:6px 10px; font-size:0.65rem;">SET CH B</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="module-box" style="display: flex; flex-direction: column; justify-content: space-between;">
                <div>
                    ...
                    <div style="margin-top: 5px; border-bottom: 1px solid var(--border); padding-bottom: 20px;">
                        ...
                        <span style="display:block; font-size:0.65rem; color:var(--text-muted); text-align:center; margin-top:10px; font-family: monospace;">Spectrum: 20Hz - 20kHz (Ideal untuk Cek Phase)</span>
                    </div>
                </div>
    ...
    <div id="dsp-container">
        ...
        <div id="tab-cha" class="tab-content module-box">
            ...
            <!-- FITUR BARU: SPEAKER PHASE CHECKER CH A -->
            <div class="phase-check-box">
                <div class="phase-status">
                    <span id="phase-led-l" class="phase-led"></span>
                    <span>PHASE: <strong id="phase-text-l" style="color:#fff;">STANDBY</strong></span>
                </div>
                <button id="btn-phase-check-l" class="btn-link" style="padding:6px 12px; font-size:0.7rem;">🔍 PHASE CHECK</button>
            </div>
        ...
        <div id="tab-chb" class="tab-content module-box">
            ...
            <!-- FITUR BARU: SPEAKER PHASE CHECKER CH B -->
            <div class="phase-check-box">
                <div class="phase-status">
                    <span id="phase-led-r" class="phase-led"></span>
                    <span>PHASE: <strong id="phase-text-r" style="color:#fff;">STANDBY</strong></span>
                </div>
                <button id="btn-phase-check-r" class="btn-link" style="padding:6px 12px; font-size:0.7rem;">🔍 PHASE CHECK</button>
            </div>
...
<script>
    ...
    let hpfL = [], lpfL = [], peqL = [], delayL, apfL;
    let hpfR = [], lpfR = [], peqR = [], delayR, apfR;
    ...
    let isCheckingPhaseL = false, isCheckingPhaseR = false;
    ...
    function initAudio() {
        ...
        peqL[4].connect(delayL); delayL.connect(limiterL); limiterL.connect(gainL); gainL.connect(analyserL); gainL.connect(merger, 0, 0);
        ...
        peqR[4].connect(delayR); delayR.connect(limiterR); limiterR.connect(gainR); gainR.connect(analyserR); gainR.connect(merger, 0, 1);
    ...
    // LOGIKA SPEAKER PHASE CHECKER (REALTIME ASYMMETRY ANALYSIS)
    function runPhaseChecker(ch) {
        initAudio();
        if(audioCtx.state === 'suspended') audioCtx.resume();
        
        const led = document.getElementById(`phase-led-${ch}`);
        const text = document.getElementById(`phase-text-${ch}`);
        const analyser = ch === 'l' ? analyserL : analyserR;
        const dataArr = ch === 'l' ? dataArrL : dataArrR;
        
        let isChecking = ch === 'l' ? isCheckingPhaseL : isCheckingPhaseR;
        if(isChecking) return; 
        
        if(ch === 'l') isCheckingPhaseL = true; else isCheckingPhaseR = true;
        
        led.className = "phase-led checking";
        text.innerText = "CHECKING...";
        text.style.color = "var(--led-yellow)";
        ...
        const analyzeFrame = () => {
            ...
            for(let i=0; i<dataArr.length; i++) {
                if(dataArr[i] > maxPos) maxPos = dataArr[i];
                if(dataArr[i] < minNeg) minNeg = dataArr[i];
            }
            
            if(Math.abs(maxPos) > Math.abs(minNeg)) {
                posPeaks += Math.abs(maxPos);
            } else {
                negPeaks += Math.abs(minNeg);
            }
            ...
            if(samplesCollected < 30) {
                ...
                let isInPhase = posPeaks >= negPeaks;
                if(isInPhase) {
                    led.className = "phase-led in-phase";
                    text.innerText = "+ NORMAL (IN PHASE)";
                    text.style.color = "var(--led-green)";
                } else {
                    led.className = "phase-led out-phase";
                    text.innerText = "- TERBALIK (OUT OF PHASE)";
                    text.style.color = "var(--led-red)";
                }
                if(ch === 'l') isCheckingPhaseL = false; else isCheckingPhaseR = false;
            }
        };
        analyzeFrame();
    }

    document.getElementById('btn-phase-check-l').addEventListener('click', () => runPhaseChecker('l'));
    document.getElementById('btn-phase-check-r').addEventListener('click', () => runPhaseChecker('r'));
